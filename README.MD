...
Mapping condensed into one variable?
Lack of param validation / access control
...
data.length being abused to pass different types of validations
It's the same line coverage, but it's different values. So in a sense the "line coverage" that is different is in the value?

----

Logic mistake / Incorrect value 
-> Value returned is wrong
-> Real value is right
-> Mistake has no reflection in coverage, this is purely logical / spec based.

----

Bunni exploit

Altered ratio -> Creating an imaginary amount of liquidity?
...
The address belongs to a contract, it doesn't, it's a precompile or an EOA.
The address is the caller or the tx.origin.
2 or more addresses can have relations:
- Same address
...
They are different
Of similar scale (where scale is subjective but objectively scale is the remainder / precision loss from them)
Scale of the precision loss they generate
